...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const [showHRAccount, setShowHRAccount] = useState(false);
  ...
  return (
    <>
      <div className={`fixed left-0 top-0 h-full bg-white border-r border-gray-200 shadow-sm transition-all duration-300 z-50 ${
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200">
          ...
          {collapsed && (
            <button
              onClick={() => setShowHRAccount(true)}
              className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-700 transition-colors cursor-pointer mx-auto"
              title="HR账号设置"
            >
              <span className="text-white font-bold text-sm">HR</span>
            </button>
      ...
      {/* HR Account Modal */}
      {showHRAccount && (
        <HRAccountModal onClose={() => setShowHRAccount(false)} />
      )}
    </>
  );
};
